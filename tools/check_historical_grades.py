...
def main() -> None:
    ...
    # historical data doesn't include ruin_probability; set to 0.0 (optimistic)
    df["ruin_probability"] = 0.0

    # compute risk-adjusted EV and scores
    ...
    print("Grade counts (after veto):")
    for g in ["S", "A", "B", "C", "D", "E"]:
        print(f"  {g}: {counts.get(g, 0)}")

    # Show any rows with D or better
    positive = df[df["grade"].isin(["D", "C", "B", "A", "S"])].copy()
    if positive.empty:
        print("No historical rows with grade D or higher were found.")
    else:
        print(
            f"Found {len(positive)} rows with grade >= D. Showing top 10 by composite_score:"
        )
